...
def smoothing_singal(signal_data):
...
def plot_signal(ecg_signal, r_peaks, q_points, s_points, p_points, t_points, p_onsets, q_onsets, s_offsets, t_offsets):
...
def read_signal(path):
    # Read the signal data in the given path
    header_file = path
    header = wfdb.rdheader(header_file)
    record_path = path
    record = wfdb.rdrecord(record_path) 
    return record.fs, record.p_signal 

if __name__ == "__main__":
    # Read the header file
    path = 'Data_Processing/data/s0010_re'
    fs, signal = read_signal(path)
    smoothed_signal = smoothing_singal(signal)
    for i in range(smoothed_signal.shape[1]):
        ...
        csv_path = f"CSV_Files/ECG_Channel_{i+1}.csv"
        df_features.to_csv(csv_path, sep="t")
